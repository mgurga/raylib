...
class Color : public ::Color {
 public:
    ...
    inline void RayDrawText(const std::string& text, int posX = 0, int posY = 0, int fontSize = 10.0f) const {
        ::RayDrawText(text.c_str(), posX, posY, fontSize, *this);
    }

    inline void RayDrawText(const ::Font& font, const std::string& text, ::Vector2 position,
            float fontSize, float spacing) const {
        ::RayDrawTextEx(font, text.c_str(), position, fontSize, spacing, *this);
    }

    inline void RayDrawText(
            const ::Font& font,
    ...
    inline void DrawRayRectangle(int posX, int posY, int width, int height) const {
        ::DrawRayRectangle(posX, posY, width, height, *this);
    }

    inline void DrawRayRectangle(::Vector2 position, ::Vector2 size) const {
        ::DrawRayRectangleV(position, size, *this);
    }

    inline void DrawRayRectangle(::RayRectangle rec) const {
        ::DrawRayRectangleRec(rec, *this);
    }

    inline void DrawRayRectangle(::RayRectangle rec, ::Vector2 origin, float rotation) const {
        ::DrawRayRectanglePro(rec, origin, rotation, *this);
    }

    inline void DrawRayRectangleLines(int posX, int posY, int width, int height) const {
        ::DrawRayRectangleLines(posX, posY, width, height, *this);
    }

    inline void DrawRayRectangleLines(::RayRectangle rec, float lineThick) const {
        ::DrawRayRectangleLinesEx(rec, lineThick, *this);
    }
