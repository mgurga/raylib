 ...
 */
RLCPPAPI inline void RayDrawText(const std::string& text, int posX, int posY, int fontSize, ::Color color) {
    ::RayDrawText(text.c_str(), posX, posY, fontSize, color);
}

/**
 * Draw text using font and additional parameters
 */
RLCPPAPI inline void RayDrawTextEx(const Font& font, const std::string& text, Vector2 position,
        float fontSize, float spacing, ::Color tint) {
    ::RayDrawTextEx(font, text.c_str(), position, fontSize, spacing, tint);
}
