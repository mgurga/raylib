...
RMAPI Matrix MatrixFrustum(double left, double right, double bottom, double top, double raynear, double rayfar)
{
    Matrix result = { 0 };

    float rl = (float)(right - left);
    float tb = (float)(top - bottom);
    float fn = (float)(rayfar - raynear);

    result.m0 = ((float)raynear*2.0f)/rl;
    result.m1 = 0.0f;
    result.m2 = 0.0f;
    result.m3 = 0.0f;

    result.m4 = 0.0f;
    result.m5 = ((float)raynear*2.0f)/tb;
    result.m6 = 0.0f;
    result.m7 = 0.0f;

    result.m8 = ((float)right + (float)left)/rl;
    result.m9 = ((float)top + (float)bottom)/tb;
    result.m10 = -((float)rayfar + (float)raynear)/fn;
    result.m11 = -1.0f;

    result.m12 = 0.0f;
    result.m13 = 0.0f;
    result.m14 = -((float)rayfar*(float)raynear*2.0f)/fn;
    result.m15 = 0.0f;
...
RMAPI Matrix MatrixPerspective(double fovy, double aspect, double raynear, double rayfar)
{
    Matrix result = { 0 };

    double top = raynear*tan(fovy*0.5);
    double bottom = -top;
    double right = top*aspect;
    double left = -right;

    // MatrixFrustum(-right, right, -top, top, raynear, rayfar);
    float rl = (float)(right - left);
    float tb = (float)(top - bottom);
    float fn = (float)(rayfar - raynear);
...
RMAPI Matrix MatrixOrtho(double left, double right, double bottom, double top, double raynear, double rayfar)
{
    Matrix result = { 0 };
    ...
    result.m14 = -((float)rayfar + (float)raynear)/fn;
    result.m15 = 1.0f;
